...
public class Crc8Util {
    ...
    public static byte computeBestCrc(final byte[] data, int length) {
        byte result = 0x00;
        for (int index = 0; index < length - 1; index++) {
            result = CHECKSUM_TABLE[(result ^ data[index]) & 0xFF];
        }
        return result;
    }

    public static byte computeBestCrc(final byte[] data, int bufferLength, int commandLength, byte type) {
        byte result = 0x07;
        result = CHECKSUM_TABLE[(result ^ commandLength & 0XFF) & 0xFF];
        result = CHECKSUM_TABLE[(result ^ commandLength / 0xFF) & 0xFF];
        result = CHECKSUM_TABLE[(result ^ type) & 0xFF];
