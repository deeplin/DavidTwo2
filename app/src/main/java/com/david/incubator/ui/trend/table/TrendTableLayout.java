package com.david.incubator.ui.trend.table;
...
import com.david.databinding.LayoutTrendTableBinding;
...
public class TrendTableLayout extends BindingBasicLayout<LayoutTrendTableBinding> {
    ...
    public TrendTableLayout(Context context) {
        super(context);
        binding.titleView.set(R.string.trend_table, LayoutPageEnum.SYSTEM_HOME, true);
        ...
        binding.trendPrint.setOnClickListener(v -> systemModel.showLayout(LayoutPageEnum.TREND_PRINT));
        binding.trendSwitch.setOnClickListener(v -> systemModel.showLayout(LayoutPageEnum.TREND_CHART));

        binding.pageUp.setOnClickListener(v1 -> {
            offset--;
            refresh();
        });

        binding.pageDown.setOnClickListener(v -> {
            offset++;
            refresh();
        });
    }
    ...
    protected int getLayoutId() {
        return R.layout.layout_trend_table;
    }
            ...
            emitter.onComplete();
                ...
                .subscribe(new Observer<BaseEntity>() {
                    ...
                    public void onComplete() {
                        binding.pageUp.setEnabled(offset > 0);
                        binding.pageDown.setEnabled(itemId > ROW_COUNT);
                        if (itemId < ROW_COUNT) {
