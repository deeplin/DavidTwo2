...
import com.david.databinding.LayoutTimingSmallBinding;
import com.david.incubator.ui.home.standard.top.TimingData;

import javax.inject.Inject;

public class TimingSmallLayout extends BindingBasicLayout<LayoutTimingSmallBinding> {
    ...
    public TimingSmallLayout(Context context, AttributeSet attrs) {
        super(context, attrs);
        ContextUtil.getComponent().inject(this);
        binding.setViewModel(timingData);
        binding.timingView.setSmallLayout();

        binding.startButton.setOnClickListener(v -> {
            binding.startButton.setVisibility(INVISIBLE);
            binding.timeString.setVisibility(VISIBLE);
            binding.timeString.setTextSize(timingData.isApgar() ? 40 : 30);
    ...
    protected int getLayoutId() {
        return R.layout.layout_timing_small;
    }
