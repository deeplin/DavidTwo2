...
public class CameraImageListLayout extends BindingLayout<LayoutImageListBinding> {
    ...
    public CameraImageListLayout(Context context) {
        super(context);
        ContextUtil.getComponent().inject(this);
        super.init(LayoutPageEnum.CAMERA_IMAGE_LIST);

        boolean error = false;

        FileUtil.makeDirectory(CameraUtil.buildDirectory(CameraUtil.IMAGE_DIRECTORY));
        ...
        if (!error) {
            for (int index = 0; index < Constant.USER_PER_PAGE; index++) {
                ...
                button.setOnClickListener(v -> {
                    ...
                    CameraImageLayout.filePath = file.getAbsolutePath();
                    systemModel.showLayout(LayoutPageEnum.CAMERA_IMAGE);
                });
    ...
    private void show() {
        ...
        Observable.create((ObservableOnSubscribe<File>) emitter -> {
            File[] files = FileUtil.listSortedFile(CameraUtil.buildDirectory(CameraUtil.IMAGE_DIRECTORY), START_TIME, END_TIME);
